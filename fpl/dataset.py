...
def clean_fpl(PATH, YEARS):
    ...
    for year in YEARS:
        ...
        raw["element_type_name"] = raw["element_type"].map(
...
def make_available_players_df(THIS_SEASON, LAST_SEASON):
    ...
    condition = (AVAILABLE["total_points"] > 0) & (
        AVAILABLE["total_points_last_season"].isna()
    )
    AVAILABLE.loc[condition, "total_points_last_season"] = AVAILABLE.loc[
        condition, "total_points"
    ]

    AVAILABLE["total_points_last_season"] = AVAILABLE[
...
def histogram_plots(df):
    ...
    for ax, position, label in zip(
        ...
        data = players_26_27[players_26_27.element_type_name == position].total_points
        sns.histplot(
            data, kde=True, ax=ax, label=label, stat="density", kde_kws={"cut": 3}
        )
...
def impute_missing_values(players_df, impute_cols):
    ...
    for idx, row in players_df[
        ...
        for col in impute_cols:
            median_value = medians[str(row["element_type_name"])][str(row["now_cost"])][
                str(col)
            ]
            std_value = stds[str(row["element_type_name"])][str(row["now_cost"])][
                str(col)
            ]
            players_df.loc[idx, col] = median_value + np.abs(
                (np.random.randn() / 1.5) * std_value
            )
...
last_season = clean_data[clean_data["season"] == "2025-26"]
this_season = clean_data[clean_data["season"] == "2026-27"]
players_26_27 = make_available_players_df(this_season, last_season)
histogram_plots(players_26_27)
players_26_27_impute = impute_missing_values(players_26_27, impute_cols)
histogram_plots(players_26_27_impute)
players_26_27_impute.to_csv("data\\processed\\2026-27\\players.csv")
